...
passport.use(new GoodreadsStrategy({
  ...
  callbackURL: process.env.GOODREADS_REDIRECT_URL,
  passReqToCallback: true
}, function(req, accessToken, refreshToken, profile, done) {
  if (req.user) {
    User.findOne({goodreads: profile.id}, (err, existingUser) => {
      if (existingUser) {
        req.flash('errors', {msg: 'There is already a Goodreads account that belongs to you. Sign in to that account.'})
        done(err);
      } else {
        User.findById(req.user.id, (err, user) => {
          user.goodreads = profile.id;
          user.tokens.push({kind: 'goodreads', accessToken});
          user.profile.name = user.profile.name || profile.displayName;
          user.profile.website = user.profile.website || profile._xml2js.user.link;
          user.save((err) => {
            req.flash('info', {msg: 'Goodreads account has been linked.'});
            done(err, user);
          });
        });
      }
    ...
    User.findOne({goodreads: profile.id}, (err, existingUser) => {
      ...
      user.tokens.push({kind: 'goodreads', accessToken});
      user.profile.name = profile.displayName;
      user.profile.website = profile._xml2js.user.link;
      user.save((err) => {
        done(err, user);
      });
    });
